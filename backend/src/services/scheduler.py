    ...
    try:
        ...
        for user in users:
            ...
            assignments = (
                ...
                .filter(UserSystemAssignment.user_id == user.id)
                .all()
            )
            ...
            all_items = (
                db.query(CheckItem).filter(CheckItem.system_id.in_(system_ids)).all()
            )
            ...
            checked_records = (
                ...
                .filter(ChecklistRecord.check_date == today)
                .all()
            )
            ...
            # 체크되지 않은 항목
            unchecked_items = [
                item for item in all_items if item.id not in checked_item_ids
            ]
            ...
            if unchecked_items:
                ...
                for item in unchecked_items:
                    system = (
                        db.query(System).filter(System.id == item.system_id).first()
                    )
                    if system:
                ...
                <html>
                <body>
                    <h2>시스템 체크리스트 미체크 항목 알림</h2>
                    <p>안녕하세요, {user.name}님. DX본부 시스템 체크리스트 안내입니다.</p>
                ...
                # 테스트 모드면 테스트 이메일로, 아니면 사용자 이메일로
                recipient_email = test_email if is_test_mode else user.email

                if is_test_mode:
                    print(
                        f"  [테스트 모드] 원래 수신자: {user.email} -> 테스트 수신자: {recipient_email}"
                    )

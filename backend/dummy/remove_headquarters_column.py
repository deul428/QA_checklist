...
def remove_headquarters_column():
    ...
    try:
        ...
        print("\n[주의] SQLite는 컬럼 삭제를 직접 지원하지 않습니다.")
        print("테이블을 재생성하여 headquarters 컬럼을 제거합니다.")
        
        # 1. 백업 테이블 생성
        timestamp = __import__('datetime').datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_table = f"users_backup_before_remove_hq_{timestamp}"
        
        print(f"\n[1단계] 백업 테이블 생성: {backup_table}")
        db.execute(text(f"""
        ...
        # 2. 새 users 테이블 생성 (headquarters 제외)
        print("\n[2단계] 새 users 테이블 생성 (headquarters 제외)...")
        db.execute(text("""
        ...
        # 4. VIEW 삭제 (users 테이블을 참조하는 VIEW들)
        print("\n[4단계] VIEW 삭제...")
        views_to_drop = [

...
import { checklistAPI, CheckItem, ChecklistRecord, ChecklistSubmitItem } from '../api/api';
...
const Checklist: React.FC = () => {
  ...
  const [existingRecords, setExistingRecords] = useState<Record<number, ChecklistRecord>>({});
  const [checklistData, setChecklistData] = useState<Record<number, 'PASS' | 'FAIL'>>({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 기존 기록을 맵으로 변환
        const recordsMap: Record<number, ChecklistRecord> = {};
        recordsData.forEach((record) => {
          recordsMap[record.check_item_id] = record;
        });
        ...
        recordsData.forEach((record) => {
          if (itemsData.some(item => item.id === record.check_item_id)) {
            initialData[record.check_item_id] = record.status as 'PASS' | 'FAIL';
            if (record.notes) {
  ...
  const handleSubmit = async () => {
    ...
    // 모든 항목이 체크되었는지 확인
    const uncheckedItems = checkItems.filter(
      (item) => !checklistData[item.id]
    );

    if (uncheckedItems.length > 0) {
      setMessage({
        type: 'error',
        text: `모든 항목을 체크해주세요. (${uncheckedItems.length}개 미체크)`,
      });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: 'success', text: '체크리스트가 성공적으로 저장되었습니다' });
                ...
                <div key={item.id} className="check-item">
                  ...
                  {item.special_notes.length > 0 && (
                    <div className="special-notes">
                      <strong>특이사항:</strong>
                      <ul>
                        {item.special_notes.map((note, idx) => (
                          <li key={idx}>{note}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="check-item-actions">
                    <div className="status-buttons">
                      <button
                        className={`status-btn ${checklistData[item.id] === 'PASS' ? 'active pass' : ''}`}
                        onClick={() => handleStatusChange(item.id, 'PASS')}
                      >
                        ✓ PASS
                      </button>
                      <button
                        className={`status-btn ${checklistData[item.id] === 'FAIL' ? 'active fail' : ''}`}

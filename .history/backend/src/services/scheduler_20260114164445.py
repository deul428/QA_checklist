...
def check_unchecked_items():
    ...
    try:
        ...
        if is_test_mode:
            print(f"[스케줄러] 테스트 모드 활성화 - 모든 이메일을 {test_email}로 발송합니다")
        ...
        for user in users:
            ...
            # 테스트 모드면 테스트 이메일로, 아니면 사용자 이메일로
                ...
                if is_test_mode:
                    print(f"  [테스트 모드] 원래 수신자: {user.email} -> 테스트 수신자: {recipient_email}")
...
def init_scheduler():
    ...
    scheduler.add_job(
        check_unchecked_items,
        trigger=CronTrigger(hour=16, minute=45, timezone='Asia/Seoul'),
        id='check_noon',
        name='오후 12시 체크리스트 확인',
        replace_existing=True
    )

...
from models.models import User, System, CheckItem, UserSystemAssignment, ChecklistRecord, SpecialNote
from services.schemas import (
    UserLogin, UserResponse, SystemResponse, CheckItemResponse,
    ChecklistSubmit, ChecklistRecordResponse, SpecialNoteResponse, PasswordChange
)
...
app = FastAPI(title="QA 체크리스트 시스템", version="1.0.0")
...
@app.get("/")
async def root():
    return {"message": "QA 체크리스트 시스템 API"}
...
async def get_check_items(
    ...
    for item in check_items:
        ...
        special_notes = db.query(SpecialNote).filter(
            SpecialNote.check_item_id == item.id,
            SpecialNote.is_active == True
        ).all()
        ...
        result.append({
            ...
            "special_notes": [note.note_text for note in special_notes]
        })
...
async def get_today_checklist(
    ...
    records = db.query(ChecklistRecord).filter(
        ChecklistRecord.user_id == current_user.id,

...
DX본부 시스템 체크리스트은 SQLite 데이터베이스를 사용하며, 사용자 관리, 시스템 관리, 체크리스트 항목 관리, 체크리스트 기록 관리를 위한 5개의 주요 테이블로 구성되어 있습니다.
...
시스템 선택
  ↓
check_items에서 system_id로 조회 (description에 특이사항 포함)
  ↓
...
```python
{
  ...
  "system_id": int,
  "item_name": "string",
  "description": "string" | null,  # 상세 설명 및 특이사항 포함
  "order_index": int
}
```
...
2. **시스템 삭제 시**: 해당 시스템의 모든 체크 항목과 사용자 할당이 자동 삭제됩니다 (CASCADE DELETE).

3. **체크 항목 삭제 시**: 해당 항목의 모든 체크리스트 기록이 자동 삭제됩니다 (CASCADE DELETE).

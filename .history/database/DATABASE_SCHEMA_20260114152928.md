...
| `description` | TEXT | NULL | 체크 항목 상세 설명 |
...
### 6. special_notes (특이사항 테이블)

체크 항목에 대한 특이사항이나 주의사항을 저장하는 테이블입니다.

| 컬럼명 | 타입 | 제약조건 | 설명 |
|--------|------|----------|------|
| `id` | INTEGER | PRIMARY KEY, AUTO INCREMENT | 특이사항 고유 ID |
| `check_item_id` | INTEGER | FOREIGN KEY, NOT NULL | 소속 체크 항목 ID (check_items.id 참조) |
| `note_text` | TEXT | NOT NULL | 특이사항 내용 |
| `is_active` | BOOLEAN | DEFAULT TRUE | 활성화 여부 (비활성화 시 숨김 처리) |
| `created_at` | DATETIME | DEFAULT CURRENT_TIMESTAMP | 생성 시간 |
| `updated_at` | DATETIME | DEFAULT CURRENT_TIMESTAMP, ON UPDATE | 수정 시간 |

**특징:**
- 하나의 체크 항목은 여러 개의 특이사항을 가질 수 있습니다
- 체크 항목이 삭제되면 해당 특이사항도 자동 삭제됩니다 (CASCADE DELETE)
- `is_active`가 `FALSE`인 경우 프론트엔드에서 표시하지 않을 수 있습니다
- 프론트엔드에서 체크리스트 작성 시 특이사항이 표시되어 사용자에게 안내합니다
...
users (1) ──< (N) user_system_assignments (N) >── (1) systems
  ...
  └──< (N) checklist_records                            │
         ...
         │ (N)                                            │ (1)
         │                                                │
         └──> (1) check_items <── (1) ──< (N) special_notes
```
...
- **check_items → special_notes**: 1대다 관계 (한 항목은 여러 특이사항을 가짐)
